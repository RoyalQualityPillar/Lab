...
import { TspicmCreateUpdateComponent } from '../tspicm-create-update/tspicm-create-update.component';
import { MessageDialogComponent } from 'src/app/common/message-dialog/message-dialog.component';
import { changeStatusByCode } from 'src/app/common/removeEmptyStrings';

@Component({
  selector: 'app-tspicm-home-page',
  standalone: false,
  templateUrl: './tspicm-home-page.component.html',
  styleUrl: './tspicm-home-page.component.scss'
})
export class TspicmHomePageComponent implements OnInit, AfterViewInit {
  ...
  allTspicmTableDataUrl: any;
  activeTspicmTableDataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  allCmrTabledataUrl: apiEndPoints;

  constructor(
    private router: Router,
    private tspicmServiceService:TspicmServiceService,
  ...
  ngOnInit(): void {
    this.allTspicmTableDataUrl = apiEndPoints.allTspicmTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.TspicmUserProfileFilterData;
    this.activeTspicmTableDataUrl = apiEndPoints.activeTspicmTabledata;
    this.params = { pageIndex, size, unitCode };
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allTspicmTableDataUrl);
      compRef.setInput('tableTitle', 'All Test Solution Preparation Input Chemical Master');
      ...
      compRef.setInput('downloadFileName', 'Test Solution Preparation Input Chemical Master');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activeTspicmTableDataUrl);
      compRef.setInput('tableTitle', 'Active Test Solution Preparation Input Chemical Master');
      ...
      console.error('Error loading Active Test Solution Preparation Input Chemical Master  table filter:', error);
    }
  }
  ...
  selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(TspicmCreateUpdateComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Chemical Preparation Code', value: this.selectedRow.uc0001 },
      { labelName: 'Chemical Code', value: this.selectedRow.ff0001 },
      { labelName: 'Chemical Name', value: this.selectedRow.ff0002 },
      { labelName: 'Therriatical qty', value: this.selectedRow.ff0003 },
      { labelName: 'Tolarance', value: this.selectedRow.ff0004 },
      { labelName: 'UOM', value: this.selectedRow.ff0005 },
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Test Solution Preparation Input Chemical Master' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Chemical Preparation Code', value: item.uc0001 },
          { labelName: 'Chemical Code', value: item.ff0001 },
          { labelName: 'Chemical Name', value: item.ff0002 },
          { labelName: 'Therriatical qty', value: item.ff0003 },
          { labelName: 'Tolarance', value: item.ff0004 },
          { labelName: 'UOM', value: item.ff0005 },
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Test Solution Preparation Input Chemical Master' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
    action: 'Action',
    uc0001: 'Chemical Preparation Code',
    ff0001: 'Chemical Code',
    ff0002: 'Chemical Name',
    ff0003: 'Therriatical qty',
    // ff0004: 'Tolarance',
    // ff0005: 'UOM',
  ...
  tableTitle: string = 'All Test Solution Preparation Input Chemical Master';
  allButtonConfig = [
