...
import { TsmCreateUpdateComponent } from '../tsm-create-update/tsm-create-update.component';
import { MessageDialogComponent } from 'src/app/common/message-dialog/message-dialog.component';
import { changeStatusByCode } from 'src/app/common/removeEmptyStrings';

@Component({
  selector: 'app-tsm-home-page',
  standalone: false,
  templateUrl: './tsm-home-page.component.html',
  styleUrl: './tsm-home-page.component.scss'
})
export class TsmHomePageComponent implements OnInit, AfterViewInit {
  ...
  allTsmTableDataUrl: any;
  activeTsmTableDataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  allTsmTabledataUrl: apiEndPoints;

  constructor(
    private router: Router,
    private tsmServiceService:TsmServiceService,
  ...
  ngOnInit(): void {
    this.allTsmTableDataUrl = apiEndPoints.allTsmTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.TsmUserProfileFilterData;
    this.activeTsmTableDataUrl = apiEndPoints.activeTsmTabledata;
    this.params = { pageIndex, size, unitCode };
    console.log('Bharat');
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allTsmTableDataUrl);
      compRef.setInput('tableTitle', 'All Test Solution Master');
      ...
      compRef.setInput('downloadFileName', 'Test Solution Master');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activeTsmTableDataUrl);
      compRef.setInput('tableTitle', 'Active Test Solution Master');
      ...
      console.error('Error loading Active  Test Solution Master table filter:', error);
    }
  }
  ...
  selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(TsmCreateUpdateComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Storage Location', value: this.selectedRow.ff0003 },
      { labelName: 'Preparation Unit qty', value: this.selectedRow.ff0004 },
      { labelName: 'Preparation UOM', value: this.selectedRow.ff0005 },
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Test Solution Master' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
    ...
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Storage Location', value: item.ff0003 },
          { labelName: 'Preparation Unit qty', value: item.ff0004 },
          { labelName: 'Preparation UOM', value: item.ff0005 },
  ...
  columnConfig = {
    ...
    ff0003: 'Storage Location',
    // ff0004: 'Preparation Unit qty',
    // ff0005: 'Preparation UOM',
  ...
  tableTitle: string = 'All Test Solution Master';
  allButtonConfig = [
