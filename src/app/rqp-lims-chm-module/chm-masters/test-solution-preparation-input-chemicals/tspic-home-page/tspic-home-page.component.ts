...
import { TspicCreateUpdateComponent } from '../tspic-create-update/tspic-create-update.component';
...
@Component({
  selector: 'app-tspic-home-page',
  standalone: false,
  templateUrl: './tspic-home-page.component.html',
  styleUrl: './tspic-home-page.component.scss'
})
export class TspicHomePageComponent implements OnInit, AfterViewInit {
  ...
  allTspicTableDataUrl: any;
  activeTspicTableDataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  allTspicTabledataUrl: apiEndPoints;

  constructor(
    private router: Router,
    private tspicServiceService:TspicServiceService,
  ...
  ngOnInit(): void {
    this.allTspicTableDataUrl = apiEndPoints.allTspicTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.TspicUserProfileFilterData;
    this.activeTspicTableDataUrl = apiEndPoints.activeTspicTabledata;
    this.params = { pageIndex, size, unitCode };
    console.log('Bharat');
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allTspicTableDataUrl);
      compRef.setInput('tableTitle', 'All Test Solution Preparation Input Chemical Record');
      ...
      compRef.setInput('downloadFileName', 'Test Solution Preparation Input Chemical Record');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activeTspicTableDataUrl);
      compRef.setInput('tableTitle', 'Active Test Solution Preparation Input Chemical Record');
      ...
      console.error('Error loading Active  Test Solution Preparation Input Chemical Record table filter:', error);
    }
  }
  ...
  selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open( TspicCreateUpdateComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Preparation Record Code', value: this.selectedRow.uc0001 },
      { labelName: 'Chemical Preparation Code', value: this.selectedRow.ff0001 },
      { labelName: 'Chemical Code', value: this.selectedRow.ff0002 },
      { labelName: 'Chemical Name', value: this.selectedRow.ff0003 },
      { labelName: 'Theritical Value', value: this.selectedRow.ff0004 },
      { labelName: 'Tolarance', value: this.selectedRow.ff0005 },
      { labelName: 'UOM', value: this.selectedRow.ff0006 },
      { labelName: 'Lot No', value: this.selectedRow.ff0007 },
      { labelName: 'Consumption qty', value: this.selectedRow.ff0008 },
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Test Solution Preparation Input Chemical Record' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Preparation Record No', value: item.uc0001 },
          { labelName: 'Chemical PreparationCode', value: item.ff0001 },
          { labelName: 'Chemical Code', value: item.ff0002 },
          { labelName: 'Chemical Name', value: item.ff0003 },
          { labelName: 'Theritical Value', value: item.ff0004 },
          { labelName: 'Tolarance', value: item.ff0005 },
          { labelName: 'UOM', value: item.ff0006 },
          { labelName: 'Lot No', value: item.ff0007 },
          { labelName: 'Consumption qty', value: item.ff0008 },
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Test Solution Preparation Input Chemical Record' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
    action: 'Action',
    uc0001: 'Preparation Record No',
    ff0001: 'Chemical Preparation Code',
    ff0002: 'Chemical Code',
    ff0003: 'Chemical Name',
    
    // ff0004: 'Theritical Value',
    // ff0005: 'Tolarance',
   //ff0006: 'UOM',
    // ff0007: 'Lot No',
    // ff0008: 'Consumption qty',
  ...
  tableTitle: string = 'All Test Solution Preparation Input Chemical Record ';
  allButtonConfig = [
