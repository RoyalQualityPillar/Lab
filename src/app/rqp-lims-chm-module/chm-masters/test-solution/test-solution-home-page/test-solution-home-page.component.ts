...
import { TestSolutionCreateUpdateComponent } from '../test-solution-create-update/test-solution-create-update.component';
...
@Component({
  selector: 'app-test-solution-home-page',
  standalone: false,
  templateUrl: './test-solution-home-page.component.html',
  styleUrl: './test-solution-home-page.component.scss'
})
export class TestSolutionHomePageComponent implements OnInit, AfterViewInit {
  ...
  allTsrTableDataUrl: any;
  activeTsrTableDataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  allTsrTabledataUrl: apiEndPoints;

  constructor(
    private router: Router,
    private testSolutionServiceService:TestSolutionServiceService,
  ...
  ngOnInit(): void {
    this.allTsrTableDataUrl = apiEndPoints.allTsrTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.TsrUserProfileFilterData;
    this.activeTsrTableDataUrl = apiEndPoints.activeTsrTabledata;
    this.params = { pageIndex, size, unitCode };
    console.log('Bharat');
  ...
  selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(TestSolutionCreateUpdateComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Preparation Record No', value: this.selectedRow.uc0001 },
      ...
      { labelName: 'Preparation qty', value: this.selectedRow.ff0006 },
    ...
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Chemical Preparation Code', value: item.ff0001 },
          ...
          { labelName: 'Preparation qty', value: item.ff0006 },
