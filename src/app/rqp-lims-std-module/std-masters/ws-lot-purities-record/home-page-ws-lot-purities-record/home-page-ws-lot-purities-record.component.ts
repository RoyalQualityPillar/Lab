...
import { CreateUpdateWsLotPuritiesRecordComponent } from '../create-update-ws-lot-purities-record/create-update-ws-lot-purities-record.component';
import { Router } from '@angular/router';

@Component({
  selector: 'app-home-page-ws-lot-purities-record',
  standalone: false,
  templateUrl: './home-page-ws-lot-purities-record.component.html',
  styleUrl: './home-page-ws-lot-purities-record.component.scss'
})
export class HomePageWsLotPuritiesRecordComponent implements OnInit, AfterViewInit {
  ...
  allWlprTableDataUrl: any;
  activeWlprTableDataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  constructor(
    ...
    private wsLotPuritiesRecordService: WsLotPuritiesRecordService,
  ...
  ngOnInit(): void {
    this.allWlprTableDataUrl = apiEndPoints.allWlprTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.WlprUserProfileFilterData;
    this.activeWlprTableDataUrl = apiEndPoints.activeWlprTabledata;
    this.params = { pageIndex, size, unitCode };
   ...
   async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this. allWlprTableDataUrl);
      compRef.setInput('tableTitle', 'Active WS Lot Purities Record');
      ...
      compRef.setInput('downloadFileName', ' WS Lot Purities Record');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activeWlprTableDataUrl);
      compRef.setInput('tableTitle', 'Active WS Lot Purities Record');
      ...
      console.error('Error loading Active WS Lot Purities Record table filter:', error);
    }
  }
  ...
  public downloadOrgDocument(row:any) {
    const templateName = 'wslpr.html';
    const moduleCode = 'STD';
    this. wsLotPuritiesRecordService.generateReport(
      row.uc0001,
      templateName,
      moduleCode
    )
  ...
  selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(CreateUpdateWsLotPuritiesRecordComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Purity Code', value: this.selectedRow.uc0001 },
      { labelName: 'WS.Lot No', value: this.selectedRow.ff0001 },
      { labelName: 'Purity Type', value: this.selectedRow.ff0002 },
      { labelName: 'Purity Value', value: this.selectedRow.ff0003 },
      { labelName: 'Value UOM', value: this.selectedRow.ff0004 },
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'WS Lot Purities Record' },
      });
      dialogRef.afterClosed().subscribe((result) => {});
    }
  }
  ...
  async onSearchAllAuditTrail() {
    ...
    if (this.selectedRow.length == 0) {
      ...
      this.isLoading = true;
      this.wsLotPuritiesRecordService
 ...
 async  structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Purity Code', value: item.uc0001 },
          { labelName: 'WS.Lot No', value: item.ff0001 },
          { labelName: 'Purity Type', value: item.ff0002 },
          { labelName: 'Purity Value', value: item.ff0003 },
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'WS Lot Purities Record' },
    });
    dialogRef.afterClosed().subscribe((result) => {});
  }
  columnConfig = {
