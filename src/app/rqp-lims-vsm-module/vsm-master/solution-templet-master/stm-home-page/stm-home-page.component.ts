...
import { StmCreateUpdateComponent } from '../stm-create-update/stm-create-update.component';

@Component({
  selector: 'app-stm-home-page',
  standalone: false,
  templateUrl: './stm-home-page.component.html',
  styleUrl: './stm-home-page.component.scss'
})
export class StmHomePageComponent    implements OnInit, AfterViewInit {
...
allSolTeTabledataUrl: any;
  activeSolTeTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  ngOnInit(): void {
    this.allSolTeTabledataUrl = apiEndPoints.allSolTeTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.SolTeUserProfileFilterData;
    this.activeSolTeTabledataUrl = apiEndPoints.activeSolTeTabledata;
    this.params = { pageIndex, size, unitCode };
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allSolTeTabledataUrl);
      compRef.setInput('tableTitle', 'All Solution templet Master');
      ...
      compRef.setInput('downloadFileName', 'Solution templet Master ');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activeSolTeTabledataUrl);
      compRef.setInput('tableTitle', 'Active Solution templet Master ');
      ...
      console.error('Error loading Active Solution templet Master  table filter:', error);
    }
  }
  ...
  selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(StmCreateUpdateComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Solution Templet Code', value: this.selectedRow.uc0001 },
      { labelName: 'Solution Code', value: this.selectedRow.ff0001 },
      { labelName: 'Solution Name', value: this.selectedRow.ff0002 },
      { labelName: 'Solution qty', value: this.selectedRow.ff0003 },
      { labelName: 'UOM', value: this.selectedRow.ff0004 },
      { labelName: 'New Preparation templet', value: this.selectedRow.ff0005 },
      { labelName: 'Standadisation templet', value: this.selectedRow.ff0006 },
      { labelName: 'Diltion', value: this.selectedRow.ff0007 },
      { labelName: 'Diltion templet', value: this.selectedRow.ff0008 },
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Solution Templet Master ' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Solution Templet Code', value: item.uc0001 },
          { labelName: 'Solution Code', value: item.ff0001 },
          { labelName: 'Solution Name', value: item.ff0002 },
          { labelName: 'Solution qty', value: item.ff0003 },
          { labelName: 'UOM', value: item.ff0004 },
          { labelName: 'New Preparation templet', value: item.ff0005 },
          { labelName: 'Standadisation templet', value: item.ff0006 },
          { labelName: 'Diltion', value: item.ff0007 },
          { labelName: 'Diltion templet', value: item.ff0008 },
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Solution Templet Master' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
    action: 'Action',
    uc0001: 'Solution Templet Code',
    ff0001: 'Solution Code',
    ff0002: 'Solution Name',
    ff0003: 'Solution qty',
    ff0004: 'UOM',
    ff0005: 'New Preparation templet',
    ff0006: 'Standadisation templet',
    ff0007: 'Diltion',
    ff0008: 'Diltion templet',
  ...
  tableTitle: string = 'All  Solution Templet Master ';
  allButtonConfig = [
