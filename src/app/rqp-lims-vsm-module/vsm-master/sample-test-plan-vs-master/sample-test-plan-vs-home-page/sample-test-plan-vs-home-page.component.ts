...
import { SampleTestPlanVsCreateUpdateComponent } from '../sample-test-plan-vs-create-update/sample-test-plan-vs-create-update.component';
import { MessageDialogComponent } from 'src/app/common/message-dialog/message-dialog.component';
import { changeStatusByCode } from 'src/app/common/removeEmptyStrings';

@Component({
  selector: 'app-sample-test-plan-vs-home-page',
  standalone: false,
  templateUrl: './sample-test-plan-vs-home-page.component.html',
  styleUrl: './sample-test-plan-vs-home-page.component.scss'
})
export class SampleTestPlanVsHomePageComponent   implements OnInit, AfterViewInit {
  ...
  allSamTpVsTabledataUrl: any;
  activeSamTpVsTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  constructor(
    ...
    private sampleRegestrationService: SampleTestPlanService,
  ...
  ngOnInit(): void {
    this.allSamTpVsTabledataUrl = apiEndPoints.allSamTpVsTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.SamTpVsUserProfileFilterData;
    this.activeSamTpVsTabledataUrl = apiEndPoints.activeSamTpVsTabledata;
    this.params = { pageIndex, size, unitCode };
    console.log('Bharat');
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allSamTpVsTabledataUrl);
      compRef.setInput('tableTitle', 'All Sample Test Plan Vs ');
      ...
      compRef.setInput('downloadFileName', ' Sample Test Plan Vs');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activeSamTpVsTabledataUrl);
      compRef.setInput('tableTitle', 'Active Sample  Test Plan Vs');
      ...
      console.error('Error loading Active Sample  Test Plan Vs Master table filter:', error);
    }
  }
  ...
  selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(SampleTestPlanVsCreateUpdateComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Test Code ', value: this.selectedRow.uc0001 },
      { labelName: 'Test Name', value: this.selectedRow.ff0001 },
      { labelName: 'Lifecycle No', value: this.selectedRow.ff0002 },
      { labelName: 'Selction', value: this.selectedRow.ff0003 },
      { labelName: 'AR.No', value: this.selectedRow.ff0004 },
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Sample Test Plan Vs' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Test Code ', value: item.uc0001 },
          { labelName: 'Test Name', value: item.ff0001 },
          { labelName: 'Lifecycle No', value: item.ff0002 },
          { labelName: 'Selction', value: item.ff0003 },
          { labelName: 'AR.No', value: item.ff0004 },
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Sample Test Plan vs ' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
  ...
  tableTitle: string = 'All Sample Test Plan vs ';
  allButtonConfig = [
