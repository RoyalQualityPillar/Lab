...
import { VsuCreateUpdateComponent } from '../vsu-create-update/vsu-create-update.component';
import { MessageDialogComponent } from 'src/app/common/message-dialog/message-dialog.component';
import { changeStatusByCode } from 'src/app/common/removeEmptyStrings';
import { Router } from '@angular/router';

@Component({
  selector: 'app-vsu-home-page',
  standalone: false,
  templateUrl: './vsu-home-page.component.html',
  styleUrl: './vsu-home-page.component.scss'
})
export class VsuHomePageComponent implements OnInit, AfterViewInit {
 ...
 allvolsolTabledataUrl: any;
 activevolsolTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
//  allvolsolTabledataUrl: apiEndPoints;
 
  
  constructor(
    private router: Router,
    private VsuService: VsuService,
  ...
  ngOnInit(): void {
    this.allvolsolTabledataUrl = apiEndPoints.allvolsolTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.volsolUserProfileFilterData;
    this.activevolsolTabledataUrl = apiEndPoints.activevolsolTabledata;
    this.params = { pageIndex, size, unitCode };
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allvolsolTabledataUrl);
      compRef.setInput('tableTitle', 'All Volumetric Solution Usage');
      ...
      compRef.setInput('downloadFileName', ' Volumetric Solution Usage');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activevolsolTabledataUrl);
      compRef.setInput('tableTitle', 'Active Volumetric Solution Usage');
      ...
      compRef.setInput('params', this.params);
      compRef.setInput('getLatestData', this.getLatestData);
      compRef.setInput('downloadFileName', 'Volumetric Solution');
      ...
      console.error('Error loading Active Volumetric Solution Master table filter:', error);
    }
  }
  ...
  selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(VsuCreateUpdateComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Volumetric Solution Usage Code', value: this.selectedRow.uc0001 },
      { labelName: 'Volumetric Solution Preparation Code', value: this.selectedRow.ff0001 },
      { labelName: 'Volumetric Solution Templet Code', value: this.selectedRow.ff0002 },
      { labelName: 'Volumetric Solution Code', value: this.selectedRow.ff0003 },
      { labelName: 'Solution Name', value: this.selectedRow.ff0004 },
      { labelName: 'Theritical Strength ', value: this.selectedRow.ff0005 },
      { labelName: 'UOM', value: this.selectedRow.ff0006 },
      { labelName: 'Solution Qty', value: this.selectedRow.ff0007 },
      { labelName: 'UOM', value: this.selectedRow.ff0008 },
      { labelName: 'Standadization Schdule ', value: this.selectedRow.ff0009 },
      { labelName: 'Batch No', value: this.selectedRow.ff0010 },
      { labelName: 'Mode Of prepation ', value: this.selectedRow.ff0011 },
      { labelName: 'Existing standadization No', value: this.selectedRow.ff0012 },
      { labelName: 'Used Type', value: this.selectedRow.ff0013 },
      { labelName: 'Used AR No ', value: this.selectedRow.ff0014 },
      { labelName: 'Used For Test Name ', value: this.selectedRow.ff0015 },
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Volumetric Solution ' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Volumetric Solution Usage Code', value: item.uc0001 },
          { labelName: 'Volumetric Solution Preparation Code', value: item.ff0001 },
          { labelName: 'Volumetric Solution Templet Code', value: item.ff0002 },
          { labelName: 'Volumetric Solution Code', value: item.ff0003 },
          { labelName: ' Solution Name', value: item.ff0004 },
          { labelName: ' Theritical Strength', value: item.ff0005 },
          { labelName: 'UOM', value: item.ff0006 },
          { labelName: 'Solution Qty', value: item.ff0007 },
          { labelName: 'UOM', value: item.ff0008 },
          { labelName: 'Standadization Schdule ', value: item.ff0009 },
          { labelName: 'Batch No', value: item.ff0010 },
          { labelName: 'Mode Of prepation', value: item.ff0011 },
          { labelName: 'Existing standadization No', value: item.ff0012 },
          { labelName: 'Used Type', value: item.ff0013 },
          { labelName: ' Used AR No', value: item.ff0014 },
          { labelName: 'Used For Test Name ', value: item.ff0015 },
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Volumetric Solution' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
    action: 'Action',
    uc0001: 'Volumetric Solution Usage Code',
    ff0001: 'Volumetric Solution Preparation Code',
    ff0002: 'Volumetric Solution Templet Code',
    ff0003: 'Volumetric Solution Code',
    // ff0004: 'Sample Type',
    // ff0005: 'STP No',
  ...
  tableTitle: string = 'All Volumetric Solution Usage';
  allButtonConfig = [
