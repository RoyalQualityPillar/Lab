...
import { SpmCreateUpdateComponent } from '../spm-create-update/spm-create-update.component';
import { MessageDialogComponent } from 'src/app/common/message-dialog/message-dialog.component';
import { changeStatusByCode } from 'src/app/common/removeEmptyStrings';

@Component({
  selector: 'app-spm-home-page',
  standalone: false,
  templateUrl: './spm-home-page.component.html',
  styleUrl: './spm-home-page.component.scss'
})
export class SpmHomePageComponent  implements OnInit, AfterViewInit {
  ...
  allSolPreTabledataUrl: any;
  activeSolPreTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  ngOnInit(): void {
    this.allSolPreTabledataUrl = apiEndPoints.allSolPreTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.SolPreUserProfileFilterData;
    this.activeSolPreTabledataUrl = apiEndPoints.activeSolPreTabledata;
    this.params = { pageIndex, size, unitCode };
    console.log('Bharat');
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allSolPreTabledataUrl);
      compRef.setInput('tableTitle', 'All Solution Preparation Master');
      ...
      compRef.setInput('downloadFileName', 'Solution Preparation Master ');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activeSolPreTabledataUrl);
      compRef.setInput('tableTitle', 'Active Solution Preparation Master');
      ...
      console.error('Error loading Active Solution Preparation Master table filter:', error);
    }
  }
  ...
  selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(SpmCreateUpdateComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Solution Preparation Code', value: this.selectedRow.uc0001 },
      { labelName: 'Solution Templet Code', value: this.selectedRow.ff0001 },
      { labelName: 'Solution Code', value: this.selectedRow.ff0002 },
      { labelName: 'Solution Name', value: this.selectedRow.ff0003 },
      { labelName: 'Solution Qty', value: this.selectedRow.ff0004 },
      { labelName: 'Batch No', value: this.selectedRow.ff0005 },
      { labelName: 'Mode Of prepation ', value: this.selectedRow.ff0006 },
      { labelName: 'Existing standadization No', value: this.selectedRow.ff0007 },
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Solution Preparation Master' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Solution Preparation Code', value: item.uc0001 },
          { labelName: 'Solution Templet Code', value: item.ff0001 },
          { labelName: 'Solution Code', value: item.ff0002 },
          { labelName: 'Solution Name', value: item.ff0003 },
          { labelName: 'Solution Qty', value: item.ff0004 },
          { labelName: 'Batch No', value: item.ff0005 },
          { labelName: 'Mode Of prepation ', value: item.ff0006 },
          { labelName: 'Existing standadization No', value: item.ff0007 },
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Solution Preparation Master' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
    action: 'Action',
    uc0001: 'Solution Preparation Code',
    ff0001: 'Solution Templet Code',
    ff0002: 'Solution Code',
    ff0003: 'Solution Name',
    ff0004: 'Solution Qty',
    ff0005: 'Batch No',
    ff0006: 'Mode Of prepation ',
    ff0007: 'Existing standadization No',
  ...
  tableTitle: string = 'All Solution Preparation Master';
  allButtonConfig = [
