...
import { SnmCreateUpdateComponent } from '../snm-create-update/snm-create-update.component';
import { MessageDialogComponent } from 'src/app/common/message-dialog/message-dialog.component';
import { changeStatusByCode } from 'src/app/common/removeEmptyStrings';
import { SnmService } from '../snm.service';

@Component({
  selector: 'app-snm-home-page',
  standalone: false,
  templateUrl: './snm-home-page.component.html',
  styleUrl: './snm-home-page.component.scss'
})
export class SnmHomePageComponent  implements OnInit, AfterViewInit {
  ...
  allSolNaTabledataUrl: any;
activeSolNaTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  ngOnInit(): void {
    this.allSolNaTabledataUrl = apiEndPoints.allSolNaTabledata;
    this.pageIndex = 0;
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allSolNaTabledataUrl);
      compRef.setInput('tableTitle', 'All Solution Name Master');
      ...
      compRef.setInput('downloadFileName', ' Solution Name Master');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activeSolNaTabledataUrl);
      compRef.setInput('tableTitle', 'Active Solution Name Master');
      ...
      console.error('Error loading Active Solution Name Master table filter:', error);
    }
  }
  ...
  selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(SnmCreateUpdateComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: ' Solution Code', value: this.selectedRow.uc0001 },
      { labelName: 'Solution Name', value: this.selectedRow.ff0001 },
      { labelName: 'Solution Type', value: this.selectedRow.ff0002 },
      { labelName: 'Solution Method No', value: this.selectedRow.ff0003 },
      { labelName: 'Expiry Period (in Days) ', value: this.selectedRow.ff0004 },
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Solution Name Master ' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: ' Solution Code', value: item.uc0001 },
          { labelName: 'Solution Name', value: item.ff0001 },
          { labelName: 'Solution Type', value: item.ff0002 },
          { labelName: 'Solution Method No', value: item.ff0003 },
          { labelName: 'Expiry Period (in Days) ', value: item.ff0004 },
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Solution Name Master ' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
    action: 'Action',
    uc0001: ' Solution Code',
    ff0001: 'Solution Name',
    ff0002: 'Solution Type',
    ff0003: 'Solution Method No',
    ff0004: 'Expiry Period (in Days)',
    // ff0005: 'STP No',
  ...
  tableTitle: string = 'All Solution Name Master';
  allButtonConfig = [
