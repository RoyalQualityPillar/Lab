...
import { FieldMasterCreateUpdateComponent } from '../field-master-create-update/field-master-create-update.component';
import { apiEndPoints } from 'src/app/service/api-service/api-endpoints.constant';
import { FieldMasterService } from '../field-master.service';

@Component({
  selector: 'app-field-master-home-page',
  standalone: false,
  templateUrl: './field-master-home-page.component.html',
  styleUrl: './field-master-home-page.component.scss'
})
export class FieldMasterHomePageComponent  implements OnInit, AfterViewInit {
  ...
  isFilterExpanded = false;
   activeFieldmasterTabledataUrl: any;
   allFieldmasterTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  ngOnInit(): void {
    this.allFieldmasterTabledataUrl =
      apiEndPoints.allFieldmasterTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.FieldmasterUserProfileFilterData;
    this.activeFieldmasterTabledataUrl =
      apiEndPoints.activeFieldmasterTabledata;
    this.params = { pageIndex, size, unitCode };
    console.log('Bharat');
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allFieldmasterTabledataUrl);
      compRef.setInput('tableTitle', 'All Field Master');
      ...
      compRef.setInput('downloadFileName', ' Field Master');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activeFieldmasterTabledataUrl);
      compRef.setInput('tableTitle', 'Active Field Master');
  ...
  selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(FieldMasterCreateUpdateComponent , {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Field No', value: this.selectedRow.uc0001 },
      { labelName: 'Field used for code', value: this.selectedRow.ff0001 },
      { labelName: 'Field used for Name', value: this.selectedRow.ff0002 },
      { labelName: 'Field Type Code', value: this.selectedRow.ff0003 },
      { labelName: 'Field Type Name ', value: this.selectedRow.ff0005 },
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Field Master' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Field No', value: item.uc0001 },
          { labelName: 'Field used for code', value: item.ff0001 },
          { labelName: 'Field used  for Name', value: item.ff0002 },
          { labelName: 'Field Type code', value: item.ff0003 },
          { labelName: 'Field Type Name ', value: item.ff0005 },
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Field Master' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
    action: 'Action',
    uc0001: 'Field No',
    ff0001: 'Field Used for code',
    ff0002: 'Field Used for Name',
    ff0004: 'Organization Unit Code',
    ff0005: 'Field Type Name',
    status: 'Status',
    version: 'Version',
    createdon: 'CreatedOn',
    createdby: 'CreatedBy',
