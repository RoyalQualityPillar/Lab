...
import { WsMasterCreateUpdateComponent } from '../ws-master-create-update/ws-master-create-update.component';

@Component({
  selector: 'app-ws-master-home-page',
  standalone: false,
  templateUrl: './ws-master-home-page.component.html',
  styleUrl: './ws-master-home-page.component.scss'
})
export class WsMasterHomePageComponent 
 implements OnInit, AfterViewInit {
  ...
  isFilterExpanded = false;
   ...
   allWsmasterTabledataUrl: any;
   activeWsmasterTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  ngOnInit(): void {
    this.allWsmasterTabledataUrl =
      apiEndPoints.allWsmasterTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.WsmasterUserProfileFilterData;
    this.activeWsmasterTabledataUrl =
      apiEndPoints.activeWsmasterTabledata;
    this.params = { pageIndex, size, unitCode };
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allWsmasterTabledataUrl);
      compRef.setInput('tableTitle', 'All Ws Master');
      ...
      compRef.setInput('downloadFileName', 'Ws Master');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activeWsmasterTabledataUrl);
      compRef.setInput('tableTitle', 'Active Ws Master');
 ...
 public downloadOrgDocument(row:any) {
    const templateName = 'wsfields.html';
    const moduleCode = 'LIMSPC';
    this.wsMasterService.generateReport(
      row.uc0001,
      templateName,
      moduleCode
    )
  ...
  selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(WsMasterCreateUpdateComponent , {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'WorkSheet No', value: this.selectedRow.uc0001 },
      { labelName: 'Field No', value: this.selectedRow.ff0001 },
      { labelName: 'Lower Limit', value: this.selectedRow.ff0002 },
      { labelName: 'Upper Limit', value: this.selectedRow.ff0003 },
      { labelName: 'Alert Limit ', value: this.selectedRow.ff0005 },
     { labelName: 'Action Limit', value: this.selectedRow.ff0006 },
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'WorkSheet No', value: item.uc0001 },
          { labelName: 'Field No', value: item.ff0001 },
          { labelName: 'Lower Limit', value: item.ff0002 },
          { labelName: 'Upper Limit', value: item.ff0003 },
          { labelName: 'Alert Limit ', value: item.ff0005 },
           { labelName: 'Action Limit', value: item.ff0006 },
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Work Sheet' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
