...
export class AddNewRecordComponent implements OnInit {
  ...
  constructor(
    ...
    private addNewRecordService: AddNewRecordService,
  ...
  ngOnInit(): void {
    ...
    this.addNewRecordService.bmrInput(this.cookieService.get('buCode')).subscribe(({ data }) => {
  ...
  public submit(): void {
    ...
    this.addNewRecordService.pmBmrUpdate(payload).subscribe((data) => {
      if (data.errorInfo != null) {
        this.dialog.open(MessageDialogComponent, {
          data: {
            message: data.errorInfo.message,
