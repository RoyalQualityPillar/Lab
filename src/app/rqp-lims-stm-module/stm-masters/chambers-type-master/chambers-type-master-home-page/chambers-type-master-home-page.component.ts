...
import { ChambersTypeMasterCreateUpdateComponent } from '../chambers-type-master-create-update/chambers-type-master-create-update.component';
...
@Component({
  selector: 'app-chambers-type-master-home-page',
  standalone: false,
  templateUrl: './chambers-type-master-home-page.component.html',
  styleUrl: './chambers-type-master-home-page.component.scss'
})
export class ChambersTypeMasterHomePageComponent implements OnInit, AfterViewInit {
  ...
  allCtmTabledataUrl: any;
  activeCtmTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  constructor(
    ...
    private chambersTypeMasterServiceService: ChambersTypeMasterServiceService,
  ...
  ngOnInit(): void {
    this.allCtmTabledataUrl = apiEndPoints.allCtmTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.CtmUserProfileFilterData;
    this. activeCtmTabledataUrl = apiEndPoints.activeCtmTabledata;
    this.params = { pageIndex, size, unitCode };
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allCtmTabledataUrl);
      compRef.setInput('tableTitle', 'All Chambers Type Master');
      ...
      compRef.setInput('downloadFileName', 'Chambers Type Master');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this. activeCtmTabledataUrl);
      compRef.setInput('tableTitle', 'All Chambers Type Master');
      ...
      console.error('Error loading Active Chambers Type Master table filter:', error);
    }
  }
  ...
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(ChambersTypeMasterCreateUpdateComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Chamber Type NO', value: this.selectedRow.uc0001 },
      { labelName: 'Chamber Type Code', value: this.selectedRow.ff0001 },
      { labelName: 'Chamber Type Name', value: this.selectedRow.ff0002},
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Chambers Type Master' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async onSearchAllAuditTrail() {
    ...
    if (this.selectedRow.length == 0) {
      ...
      this.chambersTypeMasterServiceService
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Chamber Type NO', value: item.uc0001 },
          { labelName: 'Chamber Type Code', value: item.ff0001 },
          { labelName: 'Chamber Type Name', value: item.ff0002},
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Chambers Type Master' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
    action: 'Action',
    uc0001: 'Chamber Type NO',
    ff0001: 'Chamber Type Code',
    ff0002: 'Chamber Type Name',
    // ff0003: 'No Of racks',
    // ff0004: 'No Of Shelfs',
    // ff0005: 'STP No',
  ...
  tableTitle: string = 'All Chambers Type Master';
  allButtonConfig = [
