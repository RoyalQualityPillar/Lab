...
@Component({
  selector: 'app-stability-prot-sche-rec-create-update',
  standalone: false,
  templateUrl: './stability-prot-sche-rec-create-update.component.html',
  styleUrl: './stability-prot-sche-rec-create-update.component.scss'
})
export class StabilityProtScheRecCreateUpdateComponent implements OnInit {
  isReadOnly = true;
  isUpdate = false;
  ...
  onLoadFormValue() {
    ...
    this.apiService
      .sendRequest(apiEndPoints.SpsrLoadUpdatePage, 'POST', params)
      .subscribe((data: any) => {
        if (data.data == null) {
          this.isLoading = false;
