...
import { WsPeraamentersRecordCreateUpdateComponent } from '../ws-peraamenters-record-create-update/ws-peraamenters-record-create-update.component';

@Component({
  selector: 'app-ws-peraamenters-record-home-page',
  standalone: false,
  templateUrl: './ws-peraamenters-record-home-page.component.html',
  styleUrl: './ws-peraamenters-record-home-page.component.scss'
})
export class WsPeraamentersRecordHomePageComponent implements OnInit, AfterViewInit {
  ...
  allWsPeraamentersRecordTabledataUrl: any;
  activeWsPeraametersRecordTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  constructor(
    ...
    private wsPeraamentersRecordService:WsPeraamentersRecordService ,
  ...
  ngOnInit(): void {
    this.allWsPeraamentersRecordTabledataUrl = apiEndPoints.allWsPeraamertersrecordTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.  WsPeraamentersRecordUserProfileFilterData
;
    this. activeWsPeraametersRecordTabledataUrl = apiEndPoints.allWsPeraamertersrecordTabledata;
    this.params = { pageIndex, size, unitCode };
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allWsPeraamentersRecordTabledataUrl);
      compRef.setInput('tableTitle', 'All Ws Peraamenters record');
      ...
      compRef.setInput('downloadFileName', 'Ws Peraamenters record');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this. activeWsPeraametersRecordTabledataUrl);
      compRef.setInput('tableTitle', 'All Ws Peraamenters record');
      ...
      console.error('Error loading Active Ws Peraamenters record table filter:', error);
    }
  }
  ...
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(WsPeraamentersRecordCreateUpdateComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'COA Perameter Record No', value: this.selectedRow.uc0001 },
      { labelName: 'COA  Perameter Code', value: this.selectedRow.ff0001 },
      { labelName: 'COA  Perameter Name', value: this.selectedRow.ff0002},
      { labelName: 'Heder/Fottor', value: this.selectedRow.ff0003},
      { labelName: 'COA templet No', value: this.selectedRow.ff0004},
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Ws Peraamenters record' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async onSearchAllAuditTrail() {
    ...
    if (this.selectedRow.length == 0) {
      ...
      this.wsPeraamentersRecordService
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'COA Perameter Record No', value: item.uc0001 },
          { labelName: 'COA  Perameter Code', value: item.ff0001 },
          { labelName: 'COA  Perameter Name', value: item.ff0002},
           { labelName: 'Heder/Fottor', value: item.ff0003},
            { labelName: 'COA templet No', value: item.ff0004},
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Ws Peraamenters record' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
    action: 'Action',
    uc0001: 'COA Perameter Record No',
    ff0001: 'COA  Perameter Code',
    ff0002: 'COA  Perameter Name',
    ff0003: 'Heder/Fottor',
    ff0004: 'COA templet No',
    ff0005: 'STP No',
  ...
  tableTitle: string = 'All Ws Peraamenters record';
  allButtonConfig = [
