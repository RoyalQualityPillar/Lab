...
import { ChambersMasterCreateUpdateComponent } from '../../chambers-master/chambers-master-create-update/chambers-master-create-update.component';
import { changeStatusByCode } from 'src/app/common/removeEmptyStrings';
import { ChambersShellMasterServiceService } from '../chambers-shell-master-service.service';

@Component({
  selector: 'app-chambers-shell-master-home-page',
  standalone: false,
  templateUrl: './chambers-shell-master-home-page.component.html',
  styleUrl: './chambers-shell-master-home-page.component.scss'
})
export class ChambersShellMasterHomePageComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    private chambersShellMasterServiceService: ChambersShellMasterServiceService,
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('downloadFileName', 'Chambers Shell Master');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      console.error('Error loading Active Chambers Shell Master table filter:', error);
    }
  }
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Shell No', value: this.selectedRow.uc0001 },
      { labelName: 'Chamber No', value: this.selectedRow.ff0001 },
      { labelName: 'Rack Name', value: this.selectedRow.ff0002},
      { labelName: 'Rack No', value: this.selectedRow.ff0003},
      { labelName: 'Shell Name', value: this.selectedRow.ff0004},
  ...
  async onSearchAllAuditTrail() {
    ...
    if (this.selectedRow.length == 0) {
      ...
      this.chambersShellMasterServiceService
    ...
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Shell No', value: item.uc0001 },
          { labelName: 'Chamber No', value: item.ff0001 },
          { labelName: 'Rack Name', value: item.ff0002},
           { labelName: 'Rack No', value: item.ff0003},
            { labelName: 'Shell Name', value: item.ff0004},
  ...
  tableTitle: string = 'All Chambers Shell Master';
  allButtonConfig = [
