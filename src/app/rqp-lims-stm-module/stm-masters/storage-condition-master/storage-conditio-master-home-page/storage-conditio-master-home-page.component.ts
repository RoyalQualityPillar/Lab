...
import { StorageConditioMasterCreateUpdateComponent } from '../storage-conditio-master-create-update/storage-conditio-master-create-update.component';
import { StorageConditioMasterServiceService } from '../storage-conditio-master-service.service';

@Component({
  selector: 'app-storage-conditio-master-home-page',
  standalone: false,
  templateUrl: './storage-conditio-master-home-page.component.html',
  styleUrl: './storage-conditio-master-home-page.component.scss'
})
export class StorageConditioMasterHomePageComponent   implements OnInit, AfterViewInit {
  ...
  allScmaTabledataUrl: any;
  activeScmaTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  constructor(
    ...
    private storageConditioMasterServiceService: StorageConditioMasterServiceService,
  ...
  ngOnInit(): void {
    this.allScmaTabledataUrl = apiEndPoints.allScmaTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.ScmaUserProfileFilterData;
    this. activeScmaTabledataUrl = apiEndPoints.activeScmaTabledata;
    this.params = { pageIndex, size, unitCode };
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allScmaTabledataUrl);
      compRef.setInput('tableTitle', 'All Storage Condition Master');
      ...
      compRef.setInput('downloadFileName', 'Storage Condition Master');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this. activeScmaTabledataUrl);
      compRef.setInput('tableTitle', 'All Storage Condition Master');
      ...
      console.error('Error loading Active Storage Condition Master table filter:', error);
    }
  }
  ...
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(StorageConditioMasterCreateUpdateComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Study Type No', value: this.selectedRow.uc0001 },
      { labelName: 'Condition Code', value: this.selectedRow.ff0001 },
      { labelName: 'Condition Name *', value: this.selectedRow.ff0002},
      { labelName: 'Study Type ', value: this.selectedRow.ff0003},
      { labelName: 'Temperature Set Point (°C) *', value: this.selectedRow.ff0004},
      { labelName: 'Temperature Lower Limit (°C)', value: this.selectedRow.ff0005},
      { labelName: 'Temperature Upper Limit (°C)', value: this.selectedRow.ff0006},
      { labelName: 'RH Set Point (%)', value: this.selectedRow.ff0007},
      { labelName: 'RH Lower Limit (%)', value: this.selectedRow.ff0008},
      { labelName: 'RH Upper Limit (%)', value: this.selectedRow.ff0009},
      { labelName: 'Light Condition', value: this.selectedRow.ff0010},
      { labelName: 'Chamber Type', value: this.selectedRow.ff0011},
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Storage Condition Master' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async onSearchAllAuditTrail() {
    ...
    if (this.selectedRow.length == 0) {
      ...
      this.storageConditioMasterServiceService
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Study Type No', value: item.uc0001 },
          { labelName: 'Condition Code', value: item.ff0001 },
          { labelName: 'Condition Name *', value: item.ff0002},
           { labelName: 'Study Type ', value: item.ff0003},
            { labelName: 'Temperature Set Point (°C) *', value: item.ff0004},
            { labelName: 'Temperature Lower Limit (°C)', value: item.ff0005},
            { labelName: 'Temperature Upper Limit (°C)', value: item.ff0006},
          { labelName: 'RH Set Point (%)', value: item.ff0007},            
          { labelName: 'RH Lower Limit (%)', value: item.ff0008},
          { labelName: 'RH Upper Limit (%)', value: item.ff0009},
          { labelName: 'Light Condition', value: item.ff0010},
         { labelName: 'Chamber Type', value: item.ff0011},
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Storage Condition Master' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
    action: 'Action',
    uc0001: 'Study Type No',
    ff0001: 'Condition Code',
    ff0002: 'Condition Name *',
    ff0003: 'Study Type ',
    ff0004: 'Temperature Set Point (°C) *',
     ff0005: 'Temperature Lower Limit (°C)',
     ff0006: 'Temperature Upper Limit (°C)',
     ff0007: 'RH Set Point (%)',
     ff0008: 'RH Lower Limit (%)',
     ff0009: 'RH Upper Limit (%)',
     ff0010: 'Light Condition',
    ff0011: 'Chamber Type',
  ...
  tableTitle: string = 'All Storage Condition Master';
  allButtonConfig = [
