...
import { WsStudyTypeCreateUpdateComponent } from '../ws-study-type-create-update/ws-study-type-create-update.component';
import { changeStatusByCode } from 'src/app/common/removeEmptyStrings';

@Component({
  selector: 'app-ws-study-type-home-page',
  standalone: false,
  templateUrl: './ws-study-type-home-page.component.html',
  styleUrl: './ws-study-type-home-page.component.scss'
})
export class WsStudyTypeHomePageComponent implements OnInit, AfterViewInit {
  ...
  allStudyTypeTabledataUrl: any;
  activeStudyTypeTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  constructor(
    ...
    private studyTypemastersterService: StudyTypeMasterService,
  ...
  ngOnInit(): void {
    this.allStudyTypeTabledataUrl = apiEndPoints.allStudyTypeTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.StudyTypeUserProfileFilterData;
    this.activeStudyTypeTabledataUrl = apiEndPoints.activeStudyTypeTabledata;
    this.params = { pageIndex, size, unitCode };
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allStudyTypeTabledataUrl);
      compRef.setInput('tableTitle', 'All Study Type');
      ...
      compRef.setInput('downloadFileName', 'Study Type');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activeStudyTypeTabledataUrl);
      compRef.setInput('tableTitle', 'All Study Type');
      ...
      console.error('Error loading Active Study Type table filter:', error);
    }
  }
  ...
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(WsStudyTypeCreateUpdateComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Study Type Code', value: this.selectedRow.ff0001 },
      { labelName: 'Study Type Name', value: this.selectedRow.ff0002},
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Study Type' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async onSearchAllAuditTrail() {
    ...
    if (this.selectedRow.length == 0) {
      ...
      this.studyTypemastersterService
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Study Type Code', value: item.ff0001 },
          { labelName: 'Study Type Name', value: item.ff0002},
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Study Type' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
    ...
    ff0001: 'Study Type Code',
    ff0002: 'Study Type Name',
    // ff0003: 'No Of racks',
    // ff0004: 'No Of Shelfs',
    // ff0005: 'STP No',
  ...
  tableTitle: string = 'All Study type';
  allButtonConfig = [
