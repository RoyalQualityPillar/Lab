...
import { WsPeramentersCreateUpdateComponent } from '../../ws-peramenters-master/ws-peramenters-create-update/ws-peramenters-create-update.component';
import { WsTemplateIndexCreateUpdateComponent } from '../ws-template-index-create-update/ws-template-index-create-update.component';

@Component({
  selector: 'app-ws-template-index-home-page',
  standalone: false,
  templateUrl: './ws-template-index-home-page.component.html',
  styleUrl: './ws-template-index-home-page.component.scss'
})
export class WsTemplateIndexHomePageComponent  implements OnInit, AfterViewInit {
  ...
  allWsTemplateIndexTabledataUrl: any;
  activeWstemplateIndexTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  private wsTemplateIndexService : WsTemplateIndexService ,
  ...
  ngOnInit(): void {
    this.allWsTemplateIndexTabledataUrl = apiEndPoints.allWsTemplateIndexTabledata;
    this.pageIndex = 0;
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allWsTemplateIndexTabledataUrl);
      compRef.setInput('tableTitle', 'All Ws Template Index');
      ...
      compRef.setInput('downloadFileName', ' Ws Template Index');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this. activeWstemplateIndexTabledataUrl);
      compRef.setInput('tableTitle', 'All  Ws Template Index');
      ...
      compRef.setInput('params', this.params);
      compRef.setInput('getLatestData', this.getLatestData);
      compRef.setInput('downloadFileName', ' Ws Template Indexr');
      ...
      console.error('Error loading Active  Ws Template Index table filter:', error);
    }
  }
  ...
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(WsTemplateIndexCreateUpdateComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'COA templet No', value: this.selectedRow.uc0001 },
      { labelName: 'COA Code', value: this.selectedRow.ff0001 },
      { labelName: 'COA Name', value: this.selectedRow.ff0002},
      { labelName: 'Sample Type', value: this.selectedRow.ff0003},
      { labelName: 'Formate No', value: this.selectedRow.ff0004},
      { labelName: 'Header Rows', value: this.selectedRow.ff0005},
      { labelName: 'Header Columans', value: this.selectedRow.ff0006},
      { labelName: 'Footer Rows', value: this.selectedRow.ff0007},
      { labelName: 'Footer Columans', value: this.selectedRow.ff0008},
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: ' Ws Template Index' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'COA templet No', value: item.uc0001 },
          { labelName: 'COA Code', value: item.ff0001 },
          { labelName: 'COA Name', value: item.ff0002},
           { labelName: 'Sample Type', value: item.ff0003},
            { labelName: 'Formate No', value: item.ff0004},
             { labelName: 'Header Rows ', value: item.ff0005},
              { labelName: 'Header Columans', value: item.ff0006},
               { labelName: 'Footer Rows ', value: item.ff0007},
                { labelName: 'Footer Columans', value: item.ff0008},
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: ' Ws Template Index' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
    action: 'Action',
    uc0001: 'COA templet No',
    ff0001: 'COA Code',
    ff0002: 'COQ Name',
    ff0003: 'Sample Type',
    ff0004: 'Formate No',
    ff0005: 'Header Rows',
    ff0006: 'Header Columans',
    ff0007: 'Footer Rows',
    ff0008: 'Footer Columans',
  ...
  tableTitle: string = 'All  Ws Template Index';
  allButtonConfig = [
