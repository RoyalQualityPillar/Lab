...
import { WsPeramentersCreateUpdateComponent } from '../ws-peramenters-create-update/ws-peramenters-create-update.component';
...
@Component({
  selector: 'app-ws-peramenters-home-page',
  standalone: false,
  templateUrl: './ws-peramenters-home-page.component.html',
  styleUrl: './ws-peramenters-home-page.component.scss'
})

export class WsPeramentersHomePageComponent  implements OnInit, AfterViewInit {
  ...
  allwsperamentersTabledataUrl: any;
  activeWsPeramentersTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  constructor(
    ...
    private wsPeramentersMasterService: WsPeramentersMasterService,
  ...
  ngOnInit(): void {
    this.allwsperamentersTabledataUrl = apiEndPoints.allWsPeramentersTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.WsPeramentersUserProfileFilterData;
    this. activeWsPeramentersTabledataUrl = apiEndPoints.activeWsPeramentersTabledata;
    this.params = { pageIndex, size, unitCode };
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allwsperamentersTabledataUrl);
      compRef.setInput('tableTitle', 'All Ws Peramenters Master');
      ...
      compRef.setInput('downloadFileName', 'Ws Peramenters Master');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this. activeWsPeramentersTabledataUrl);
      compRef.setInput('tableTitle', 'All Ws Peramenters Master');
      ...
      compRef.setInput('params', this.params);
      compRef.setInput('getLatestData', this.getLatestData);
      compRef.setInput('downloadFileName', 'Ws Peramentersers Master');
      ...
      console.error('Error loading Active Ws Peramenters Master table filter:', error);
    }
  }
  ...
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(WsPeramentersCreateUpdateComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'COA Perameter No', value: this.selectedRow.uc0001 },
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Ws Peramenters  Master' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async onSearchAllAuditTrail() {
    ...
    if (this.selectedRow.length == 0) {
      ...
      this.wsPeramentersMasterService
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'COA Perameter No', value: item.uc0001 },
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'ChaWs Peramenters Master' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
    action: 'Action',
    uc0001: 'COA Perameter Noo',
  ...
  tableTitle: string = 'All Ws Peramenters  Master';
  allButtonConfig = [
