...
import { ChambersRacksMasterCreateUpdateComponent } from '../chambers-racks-master-create-update/chambers-racks-master-create-update.component';
import { ChambersRacksMasterServiceService } from '../chambers-racks-master-service.service';

@Component({
  selector: 'app-chambers-racks-master-home-page',
  standalone: false,
  templateUrl: './chambers-racks-master-home-page.component.html',
  styleUrl: './chambers-racks-master-home-page.component.scss'
})
export class ChambersRacksMasterHomePageComponent implements OnInit, AfterViewInit {
  ...
  allCrmTabledataUrl: any;
  activeCrmTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  constructor(
    ...
    private chambersRacksMasterServiceService: ChambersRacksMasterServiceService,
  ...
  ngOnInit(): void {
    this.allCrmTabledataUrl = apiEndPoints.allCrmTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.CrmUserProfileFilterData;
    this. activeCrmTabledataUrl = apiEndPoints.activeCrmTabledata;
    this.params = { pageIndex, size, unitCode };
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allCrmTabledataUrl);
      compRef.setInput('tableTitle', 'All Chambers Racks Master');
      ...
      compRef.setInput('downloadFileName', 'Chambers Racks Master');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this. activeCrmTabledataUrl);
      compRef.setInput('tableTitle', 'All Chambers Racks Master');
      ...
      console.error('Error loading Active Chambers Racks Master table filter:', error);
    }
  }
  ...
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(ChambersRacksMasterCreateUpdateComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Rack No', value: this.selectedRow.uc0001 },
      { labelName: 'Chamber No', value: this.selectedRow.ff0001 },
      { labelName: 'Rack Name', value: this.selectedRow.ff0002},
      { labelName: 'No Of Shells', value: this.selectedRow.ff0003},
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Chambers Racks Master' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async onSearchAllAuditTrail() {
    ...
    if (this.selectedRow.length == 0) {
      ...
      this.chambersRacksMasterServiceService
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Rack No', value: item.uc0001 },
          { labelName: 'Chamber No', value: item.ff0001 },
          { labelName: 'Rack Name', value: item.ff0002},
           { labelName: 'No Of Shells', value: item.ff0003},
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Chambers Racks Master' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
  ...
  tableTitle: string = 'All Chambers Racks Master';
  allButtonConfig = [
