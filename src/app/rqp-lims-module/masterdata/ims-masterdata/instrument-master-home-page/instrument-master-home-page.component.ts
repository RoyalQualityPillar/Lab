...
import { InstrumentMasterCreateUpdateComponent } from '../instrument-master-create-update/instrument-master-create-update.component';
import { InstrumentMasterService } from '../instrument-master.service';

@Component({
  selector: 'app-instrument-master-home-page',
  standalone: false,
...
export class InstrumentMasterHomePageComponent 
 ...
 implements OnInit, AfterViewInit {
  ...
  isFilterExpanded = false;
   allinstrumentMasterTabledataUrl: any;
   activeinstrumentMasterTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  ngOnInit(): void {
    this.allinstrumentMasterTabledataUrl =
      apiEndPoints.allinstrumentMasterTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.instrumentMasterUserProfileFilterData;
    this.activeinstrumentMasterTabledataUrl =
      apiEndPoints.activeinstrumentMasterTabledata;
    this.params = { pageIndex, size, unitCode };
    console.log('Bharat');
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allinstrumentMasterTabledataUrl);
      compRef.setInput('tableTitle', 'All Instrument Master');
      ...
      compRef.setInput('downloadFileName', ' Instrument Master');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activeinstrumentMasterTabledataUrl);
      compRef.setInput('tableTitle', 'Active  Instrument Master');
  ...
  public downloadOrgDocument(row:any) {        
    const templateName = 'im.html';
    const moduleCode = 'IMS';
    this. instrumentMasterService.generateReport(
      row.uc0001,
      templateName,
      moduleCode
    )
  ...
  selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(InstrumentMasterCreateUpdateComponent , {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Instrument Master' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async structureResponse(apiResponse: any) {
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: ' Instrument Master' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
  ...
  tableTitle: string = 'All  Instrument Master';
  allButtonConfig = [
