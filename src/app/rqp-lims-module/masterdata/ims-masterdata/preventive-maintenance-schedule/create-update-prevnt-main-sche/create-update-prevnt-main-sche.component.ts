...
@Component({
  selector: 'app-create-update-prevnt-main-sche',
  standalone: false,
  templateUrl: './create-update-prevnt-main-sche.component.html',
  styleUrl: './create-update-prevnt-main-sche.component.scss'
})
export class CreateUpdatePrevntMainScheComponent implements OnInit {
  isReadOnly = true;
  isUpdate = false;
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<CreateUpdatePrevntMainScheComponent>,
  ...
  onloadDeptListDropDown() {
    this.isLoading = true;
    this.prevntMainScheService.getDropDownDeptList(this.cookieService.get('buCode')).subscribe((data: any) => {
      console.log(data);
  ...
  onLoadFormValue() {
    ...
    this.apiService
      .sendRequest(apiEndPoints.pmsMasterLoadUpdatePage, 'POST', params)
      .subscribe((data: any) => {
        if (data.data == null) {
          this.isLoading = false;
