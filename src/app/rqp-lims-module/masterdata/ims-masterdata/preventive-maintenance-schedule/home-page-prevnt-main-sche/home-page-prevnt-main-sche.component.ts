...
import { CreateUpdatePrevntMainScheComponent } from '../create-update-prevnt-main-sche/create-update-prevnt-main-sche.component';

@Component({
  selector: 'app-home-page-prevnt-main-sche',
  standalone: false,
  templateUrl: './home-page-prevnt-main-sche.component.html',
  styleUrl: './home-page-prevnt-main-sche.component.scss'
})
export class HomePagePrevntMainScheComponent implements OnInit, AfterViewInit {
  ...
  activePmsMasterTabledataUrl: any;
  allPmsMasterTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  constructor(
    ...
    private prevntMainScheService: PrevntMainScheService,
  ...
  ngOnInit(): void {
    this.allPmsMasterTabledataUrl =
      apiEndPoints.allPmsMasterTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.pmsMasterUserProfileFilterData;
    this.activePmsMasterTabledataUrl =
      apiEndPoints.activePmsMasterTabledata;
    this.params = { pageIndex, size, unitCode };
    console.log('Bharat');
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allPmsMasterTabledataUrl);
      compRef.setInput('tableTitle', 'All Preventive Maintenance Schedule');
      ...
      compRef.setInput('downloadFileName', ' Preventive Maintenance Schedule');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activePmsMasterTabledataUrl);
      compRef.setInput('tableTitle', 'Active Preventive Maintenance Schedule');
      ...
      compRef.setInput('params', this.params);
      compRef.setInput('getLatestData', this.getLatestData);
      compRef.setInput('downloadFileName', '  CalibraPreventive Maintenancetion Schedule');
  ...
  tabChanged(tabChangeEvent: any) { }
    ...
    const templateName = 'pmmcls.html';
    const moduleCode = 'IMS';
    this. prevntMainScheService.generateReport(
      row.uc0001,
      templateName,
      moduleCode
    )
  ...
  selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(CreateUpdatePrevntMainScheComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Preventive Maintenance Schedule' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async structureResponse(apiResponse: any) {
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Preventive Maintenance Schedule' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
  ...
  tableTitle: string = 'All  Preventive Maintenance Schedule';
  allButtonConfig = [
