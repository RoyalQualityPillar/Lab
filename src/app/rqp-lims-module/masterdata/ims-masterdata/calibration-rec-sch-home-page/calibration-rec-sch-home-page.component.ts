...
import { CalibrationRecSchCreateUpdateComponent } from '../calibration-rec-sch-create-update/calibration-rec-sch-create-update.component';

@Component({
  selector: 'app-calibration-rec-sch-home-page',
  standalone: false,
  templateUrl: './calibration-rec-sch-home-page.component.html',
  styleUrl: './calibration-rec-sch-home-page.component.scss'
})
export class CalibrationRecSchHomePageComponent 
 ...
 implements OnInit, AfterViewInit {
  ...
  isFilterExpanded = false;
   allcalibrationrecschMasterTabledataUrl: any;
    activecalibrationrecschMasterTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  constructor(
    private router: Router,
      private calibrationRecSchService: CalibrationRecSchService,
  ...
  ngOnInit(): void {
    this.allcalibrationrecschMasterTabledataUrl =
      apiEndPoints.allcalibrationrecschMasterTabledata;
    this.pageIndex = 0;
    ...
    this.activecalibrationrecschMasterTabledataUrl =
      apiEndPoints.activecalibrationrecschMasterTabledata;
    this.params = { pageIndex, size, unitCode };
    console.log('Bharat');
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allcalibrationrecschMasterTabledataUrl);
      compRef.setInput('tableTitle', 'All Calibration Record Schedule');
      ...
      compRef.setInput('downloadFileName', ' Calibration Record Schedule');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activecalibrationrecschMasterTabledataUrl);
      compRef.setInput('tableTitle', 'Active Calibration Record Schedule');
  ...
  public downloadOrgDocument(row:any) {        
    const templateName = 'crs.html';
    const moduleCode = 'IMS';
    this. calibrationRecSchService.generateReport(
      row.uc0001,
      templateName,
      moduleCode
    )
  ...
  selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(CalibrationRecSchCreateUpdateComponent , {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Calibration Record Schedule' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async structureResponse(apiResponse: any) {
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: ' Calibration Record Schedule' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
  ...
  tableTitle: string = 'All  Calibration Record Schedule';
  allButtonConfig = [
