...
export class CumHomePageComponent implements OnInit, AfterViewInit {
  ...
  public downloadOrgDocument(row:any) {        
    const templateName = 'cum.html';
    const moduleCode = 'IMS';
    this. cumService.generateReport(
      row.uc0001,
      templateName,
      moduleCode
    )
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Parameter No', value: this.selectedRow.ff0003 },
      { labelName: 'Parameter Name', value: this.selectedRow.ff0004 },
    ...
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Parameter No', value: item.ff0003 },
          { labelName: 'Parameter Name', value: item.ff0004 },
