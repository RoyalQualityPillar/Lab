...
import { PcmCreateUpdateComponent } from '../pcm-create-update/pcm-create-update.component';
...
@Component({
  selector: 'app-pcm-home-page',
  standalone: false,
  templateUrl: './pcm-home-page.component.html',
  styleUrl: './pcm-home-page.component.scss'
})
export class PcmHomePageComponent implements OnInit, AfterViewInit {
  ...
  allPcmMasterTabledataUrl: any;
   activePcmMasterTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  ngOnInit(): void {
    this.allPcmMasterTabledataUrl =
      apiEndPoints.allScmTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.scmUserProfileFilterData;
    this.activePcmMasterTabledataUrl =
      apiEndPoints.activeScmTabledata;
    this.params = { pageIndex, size, unitCode };
    console.log('Bharat');
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allPcmMasterTabledataUrl);
      compRef.setInput('tableTitle', 'All Pcm Master');
      ...
      compRef.setInput('downloadFileName', ' Pcm  Master');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activePcmMasterTabledataUrl);
      compRef.setInput('tableTitle', 'Active Pcm Master');
      ...
      console.error('Error loading Active Pcm Master table filter:', error);
    }
  }
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Category No', value: this.selectedRow.uc0001 },
      { labelName: 'Category Name', value: this.selectedRow.ff0001 },
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Pcm Master' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Category No', value: item.uc0001 },
          { labelName: 'Category Name', value: item.ff0001 },
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: ' Pcm Master' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
