...
export interface CalibrationScheduleRequest {
  fromDate: string;
  toDate: string;
  scheduletype: string;
}
...
@Component({
  selector: 'app-calibration-schedule-master',
  standalone: false,
  templateUrl: './calibration-schedule-master.component.html',
  styleUrl: './calibration-schedule-master.component.scss',
  providers: [
    {
      provide: DateAdapter,
...
export class CalibrationScheduleMasterComponent implements OnInit, AfterViewInit {
  ...
  isLoading = false;
  hasSearched = false;
  tableData = new MatTableDataSource<CalibrationScheduleRecord>([]);
  dateRangeError = '';
  ...
  private fetchScheduleData(): void {
    const params: CalibrationScheduleRequest = {
    ...
    this.apiService
      .sendRequest(apiEndPoints.equipmentScheduleList, 'GET', params, null)
      .subscribe({
        next: (response: any) => {
          ...
          if (response?.errorInfo) {
            ...
            const data: CalibrationScheduleRecord[] = Array.isArray(response?.data)
              ? response.data
              : [];
  ...
  private buildColumns(data: CalibrationScheduleRecord[], scheduleType: string): void {
