...
import { CreateUpdateInstrumentStatusComponent } from '../create-update-instrument-status/create-update-instrument-status.component';

@Component({
  selector: 'app-home-page-instrument-status',
  standalone: false,
  templateUrl: './home-page-instrument-status.component.html',
  styleUrl: './home-page-instrument-status.component.scss'
})
export class HomePageInstrumentStatusComponent implements OnInit, AfterViewInit {
  ...
  allInstrumentStatusTableDataUrl: any;
  activeInstrumentStatusTableDataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  ngOnInit(): void {
    this.allInstrumentStatusTableDataUrl = apiEndPoints.allIuMasterTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.IuMasterUserProfileFilterData;
    this.activeInstrumentStatusTableDataUrl = apiEndPoints.activeIuMasterTabledata;
    this.params = { pageIndex, size, unitCode };
   ...
   async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this. allInstrumentStatusTableDataUrl);
      compRef.setInput('tableTitle', 'All Instrument Status');
      ...
      compRef.setInput('downloadFileName', ' Instrument Status');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activeInstrumentStatusTableDataUrl);
      compRef.setInput('tableTitle', 'Active Instrument Status');
      ...
      console.error('Error loading Active Instrument Status table filter:', error);
    }
  }
  ...
  public downloadOrgDocument(row:any) {        
    const templateName = 'iustatus.html';
    const moduleCode = 'IMS';
    this. instrumentStatusService.generateReport(
      row.uc0001,
      templateName,
      moduleCode
    )
 ...
 selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(CreateUpdateInstrumentStatusComponent, {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Status Name No', value: this.selectedRow.uc0001 },
      { labelName: 'Status Code', value: this.selectedRow.ff0001 },
      { labelName: 'Status Name', value: this.selectedRow.ff0002 },
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Instrument Status' },
      });
      dialogRef.afterClosed().subscribe((result) => {});
    }
  }
 ...
 async  structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Status Name No', value: item.uc0001 },
          { labelName: 'Status Code', value: item.ff0001 },
          { labelName: 'Status Name', value: item.ff0002 },
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Instrument Status' },
    });
    dialogRef.afterClosed().subscribe((result) => {});
  }
  columnConfig = {
    action: 'Action',
    uc0001: 'Status Name No',
    ff0001: 'Status Code',
    ff0002: 'Status Name',
    status: 'Status',
    version: 'Version',
