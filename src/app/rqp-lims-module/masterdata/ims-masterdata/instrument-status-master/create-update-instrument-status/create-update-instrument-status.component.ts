...
@Component({
  selector: 'app-create-update-instrument-status',
  standalone: false,
  templateUrl: './create-update-instrument-status.component.html',
  styleUrl: './create-update-instrument-status.component.scss'
})
export class CreateUpdateInstrumentStatusComponent implements OnInit {
  isReadOnly = true;
  isUpdate = false;
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<CreateUpdateInstrumentStatusComponent>,
  ...
  onLoadFormValue() {
    ...
    this.apiService
      .sendRequest(apiEndPoints.IuMasterLoadUpdatePage, 'POST', params)
      .subscribe((data: any) => {
        if (data.data == null) {
          this.isLoading = false;
