...
import { MethodMasterService } from '../method-master.service';
import { MethodMasterCreateUpdateComponent } from '../method-master-create-update/method-master-create-update.component';

@Component({
  selector: 'app-method-master-home-page',
  standalone: false,
  templateUrl: './method-master-home-page.component.html',
  styleUrl: './method-master-home-page.component.scss'
})
export class MethodMasterHomePageComponent implements OnInit, AfterViewInit {
  ...
  allMethodMasterTabledataUrl: any;
   activeMethodMasterTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  constructor(
    private router: Router,
      private methodMasterService: MethodMasterService,
  ...
  ngOnInit(): void {
    this.allMethodMasterTabledataUrl =
      apiEndPoints.allMethodTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.MethodUserProfileFilterData;
    this.activeMethodMasterTabledataUrl =
      apiEndPoints.activeMethodTabledata;
    this.params = { pageIndex, size, unitCode };
    console.log('Bharat');
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allMethodMasterTabledataUrl);
      compRef.setInput('tableTitle', 'All Method Method');
      ...
      compRef.setInput('downloadFileName', ' Method Master');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activeMethodMasterTabledataUrl);
      compRef.setInput('tableTitle', 'Active Method Master');
      ...
      console.error('Error loading Active Method Master table filter:', error);
    }
  }
  ...
  public downloadOrgDocument(row:any) {        
    const templateName = 'methodMaster.html';
    const moduleCode = 'IMS';
    this. methodMasterService.generateReport(
      row.uc0001,
      templateName,
      moduleCode
    )
  ...
  selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(MethodMasterCreateUpdateComponent , {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Method Id', value: this.selectedRow.ff0001 },
      { labelName: 'Method Name', value: this.selectedRow.ff0002 },
      { labelName: 'Method Type', value: this.selectedRow.ff0003 },
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Method Master' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Method Id', value: item.ff0001 },
          { labelName: 'Method Name', value: item.ff0002 },
          { labelName: 'Method Type', value: item.ff0003 },
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Method Master' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
    action: 'Action',
    uc0001: 'Sample Run No',
    ff0001: 'Method Id',
    ff0002: 'Method Name',
    ff0003: 'Method Type',
    // ff0004: 'Analyst',
  ...
  tableTitle: string = 'All Method Master';
  allButtonConfig = [
