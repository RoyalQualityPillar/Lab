...
import { SampleRunMasterService } from '../sample-run-master.service';
import { SampleRunMasterCreateUpdateComponent } from '../sample-run-master-create-update/sample-run-master-create-update.component';

@Component({
  selector: 'app-sample-run-master-home-page',
  standalone: false,
  templateUrl: './sample-run-master-home-page.component.html',
  styleUrl: './sample-run-master-home-page.component.scss'
})
export class SampleRunMasterHomePageComponent implements OnInit, AfterViewInit {
  ...
  allSampleRunMasterTabledataUrl: any;
   activeSampleRunMasterTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  constructor(
    private router: Router,
      private sampleRunMasterService: SampleRunMasterService,
  ...
  ngOnInit(): void {
    this.allSampleRunMasterTabledataUrl =
      apiEndPoints.allSampleRunTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.SampleRunUserProfileFilterData;
    this.activeSampleRunMasterTabledataUrl =
      apiEndPoints.activeSampleRunTabledata;
    this.params = { pageIndex, size, unitCode };
    console.log('Bharat');
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allSampleRunMasterTabledataUrl);
      compRef.setInput('tableTitle', 'All Sample Run Master');
      ...
      compRef.setInput('downloadFileName', ' Sample Run Master');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activeSampleRunMasterTabledataUrl);
      compRef.setInput('tableTitle', 'Active Sample Run Master');
      ...
      console.error('Error loading Active Sample Run Master table filter:', error);
    }
  }
  ...
  tabChanged(tabChangeEvent: any) { }
   public downloadOrgDocument(row:any) {        
    const templateName = 'samplerun.html';
    const moduleCode = 'IMS';
    this. sampleRunMasterService.generateReport(
      row.uc0001,
      templateName,
      moduleCode
    )
 ...
 selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(SampleRunMasterCreateUpdateComponent , {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Sample Run No', value: this.selectedRow.uc0001 },
      { labelName: 'Run Id', value: this.selectedRow.ff0001 },
      { labelName: 'Run Name', value: this.selectedRow.ff0002 },
      { labelName: 'sample Set Id', value: this.selectedRow.ff0003 },
      { labelName: 'Instrument Id', value: this.selectedRow.ff0004 },
      { labelName: 'Acquisition Method', value: this.selectedRow.ff0005 },
      { labelName: 'Processing Method', value: this.selectedRow.ff0006 },
      { labelName: 'Run Start Time', value: this.selectedRow.ff0007 },
      { labelName: 'Run End Time', value: this.selectedRow.ff0008 },
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Sample Run Master' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Sample Run No', value: item.uc0001 },
          { labelName: 'Run Id', value: item.ff0001 },
          { labelName: 'Run Name', value: item.ff0002 },
          { labelName: 'sample Set Id', value: item.ff0003 },
          { labelName: 'Instrument Id', value: item.ff0004 },
          { labelName: 'Acquisition Method', value: item.ff0005 },
          { labelName: 'Processing Method', value: item.ff0006 },
          { labelName: 'Run Start Time', value: item.ff0007 },
          { labelName: 'Run End Time', value: item.ff0008 },
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: ' Sample Run Master' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
    ...
    ff0001: 'Run Id',
    ff0002: 'Run Name',
    ff0003: 'sample Set Id',
    ff0004: 'Instrument Id',
    // ff0005: 'Acquisition Method',
    // ff0006: 'Processing Method',
    status: 'Status',
    version: 'Version',
    createdon: 'CreatedOn',
    createdby: 'CreatedBy',
  ...
  tableTitle: string = 'All Sample Run Master';
  allButtonConfig = [
