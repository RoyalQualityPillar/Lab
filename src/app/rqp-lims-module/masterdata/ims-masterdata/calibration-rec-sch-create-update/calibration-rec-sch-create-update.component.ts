...
@Component({
  selector: 'app-calibration-rec-sch-create-update',
  standalone: false,
  templateUrl: './calibration-rec-sch-create-update.component.html',
  styleUrl: './calibration-rec-sch-create-update.component.scss'
})
export class CalibrationRecSchCreateUpdateComponent
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<CalibrationRecSchCreateUpdateComponent>,
  ...
  onloadDFListDropDown() {
    this.isLoading = true;
    this.calibrationRecSchService.getDropDownList(this.cookieService.get('buCode')).subscribe((data: any) => {
      console.log(data);
  ...
  onLoadFormValue() {
    ...
    this.apiService
      .sendRequest(apiEndPoints.calibrationrecschMasterLoadUpdatePage, 'POST', params)
      .subscribe((data: any) => {
        if (data.data == null) {
          this.isLoading = false;
