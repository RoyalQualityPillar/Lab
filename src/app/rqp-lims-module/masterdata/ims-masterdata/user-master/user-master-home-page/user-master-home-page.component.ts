...
import { UserMasterCreateUpdateComponent } from '../user-master-create-update/user-master-create-update.component';
import { Router } from '@angular/router';

@Component({
  selector: 'app-user-master-home-page',
  standalone: false,
  templateUrl: './user-master-home-page.component.html',
  styleUrl: './user-master-home-page.component.scss'
})
export class UserMasterHomePageComponent implements OnInit, AfterViewInit {
  ...
  allUserMasterTabledataUrl: any;
   activeUserMasterTabledataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  getLatestData = false;
  ...
  ngOnInit(): void {
    this.allUserMasterTabledataUrl =
      apiEndPoints.allUserTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.UserUserProfileFilterData;
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allUserMasterTabledataUrl);
      compRef.setInput('tableTitle', 'All User Master');
      ...
      compRef.setInput('downloadFileName', ' User Master');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activeUserMasterTabledataUrl);
      compRef.setInput('tableTitle', 'Active User Master');
      ...
      console.error('Error loading Active User Master table filter:', error);
    }
  }
  ...
  tabChanged(tabChangeEvent: any) { }
    ...
    const templateName = 'userMaster.html';
    const moduleCode = 'IMS';
    this. userMasterService.generateReport(
      row.uc0001,
      templateName,
      moduleCode
    )
  ...
  selectedRow: any;
  onOpenRolePOPUP() {
    const dialogRef = this.dialog.open(UserMasterCreateUpdateComponent , {
      minWidth: '80%',
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'User Id', value: this.selectedRow.ff0001 },
      { labelName: 'User Name', value: this.selectedRow.ff0002 },
      { labelName: 'Role', value: this.selectedRow.ff0003 },
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'User Master' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'User Id', value: item.ff0001 },
          { labelName: 'User Name', value: item.ff0002 },
          { labelName: 'Role', value: item.ff0003 },
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'User Master' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
  ...
  tableTitle: string = 'All User Master';
  allButtonConfig = [
