...
@Component({
  selector: 'app-rasi-init',
  templateUrl: './rasi-init.component.html',
  styleUrls: ['./rasi-init.component.scss'],
  standalone: false,
})
export class RasiInitComponent implements OnInit, OnDestroy {
  ...
  public numbers: any;
  // public numbers = this.excelHelperService.numbers;
  ...
  private forms() {
    let totalBasisContent = 0;
    let totalAnhydrousBasis = 0;
    let totalAnhydrousAndSolvent = 0;
    const {
      molecularWeightOne,
      molecularWeightTwo,
      standardAvgArea,
    ...
    for (let i = 0; i < this.getFormArray().length; i++) {
      ...
      totalBasisContent += Number(formArray.get('basisContent')?.value || 0);
      totalAnhydrousBasis += Number(formArray.get('anhydrousBasis')?.value || 0);
      totalAnhydrousAndSolvent += Number(formArray.get('ashydrousAndSolventFreeBasis')?.value || 0);
    ...
    this.rqwMaterialAssayService.save(sendValue).subscribe((data) => {
      if (data.errorInfo) {
        ...
        timer(2000)
          ...
          .subscribe(() => {
            this.router.navigateByUrl('/rqpquailtyui/excel/rasi-home');
          });
      }
    });
  }
